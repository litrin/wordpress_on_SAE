...
/*
Plugin Name: SAE Memcached
Description: Memcached backend for the WP Object Cache.
Version: 1.0
Plugin URI: http://www.litrin.net
Author: Litrin J.
...
class WP_Object_Cache {
	...
	var $default_expiration = 0; // '0' means forever
    ...
    function add($id, $data, $group = 'default', $expire = 0) {
        ...
        $result = memcache_add($this->mc, $key, $data, $expire);
	...
	function incr($id, $n = 1, $group = 'default' ) {
        ...
        memcache_increment($this->mc, $key, $n);
		return $this->cache[ $key ];
	}

	function decr($id, $n = 1, $group = 'default' ) {
		$key = $this->key($id, $group);
        memcache_decrement($this->mc, $key, $n);
	...
	function delete($id, $group = 'default') {
        ...
        $result = memcache_delete($this->mc, $key);
		...
		foreach ( $groups as $group => $ids ) {
			...
			foreach ( $ids as $id ) {
				...
				if ( isset($this->cache[$key]) ) {
					...
					$return[$key] = memcache_get($this->mc, $key);
				}
			}
		}
	...
	function replace($id, $data, $group = 'default', $expire = 0) {
		...
		if ( is_object( $data ) )
			$data = clone $data;
        $result = memcache_replace($this->mc, $key, $data, false, $expire );
		if ( false !== $result )
			$this->cache[$key] = $data;
		return $result;
	}
	...
	function set($id, $data, $group = 'default', $expire = 0) {
		...
		$result = memcache_set($this->mc, $key, $data, false, $expire);
